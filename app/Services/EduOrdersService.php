...
class EduOrdersService
{
    ...
    private function getUserWooCommerceOrders(int $userId): array
    {
        $rows = DB::table('woocommerce_order_items as oi')
            ...
            ->where('pm.meta_value', $userId)
            ->orderBy('p.post_date', 'desc')
            ->select('oi.*', 'p.post_date', 'pm.meta_key', 'pm.meta_value')
            ->get();
        ...
        foreach ($rows as $row) {
            ...
            if (isset($ordersMap[$row['order_id']])) {
                $ordersMap[$row['order_id']][$mk] = $mv;
            } else {
                $ordersMap[$row['order_id']] = $row;
                $ordersMap[$row['order_id']][$mk] = $mv;
            }
        }
        ...
        foreach ($ordersMap as $key => &$order) {
            ...
            if (strpos($name, ' - ') === false || mb_strpos($name, '逢') === false) {
                unset($ordersMap[$key]);
                continue;
            }

            $dashPos = strpos($name, ' - ');
            $productName = substr($name, 0, $dashPos);
            $pa = substr($name, $dashPos + 3);

            $fenPos = mb_strpos($pa, '逢');
            $paMonth = trim(mb_substr($pa, 0, $fenPos), " \t\n\r\v\f,");
            $paTime = trim(mb_substr($pa, $fenPos + 1), " \t\n\r\v\f,");
            $className = $productName . $paTime;

            $class = EduClass::where('class_name', $className)->first();

            $order['month'] = $paMonth;
            $order['class_name'] = $className;
            $order['class_id'] = $class?->class_id;
            $order['class_year'] = $this->getClassYear($order['post_date'] ?? '', $paMonth);
            $order['user_id'] = $order['_customer_user'] ?? $userId;
        }
        unset($order);

        return array_filter($ordersMap);
    }
