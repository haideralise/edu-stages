...
class EduService
{
    /** @var string Cache key for {@see getEduLevels()}; clear after editing `edu_level` rows. */
    public const CACHE_KEY_EDU_LEVEL_ALL = 'edu_level.all';
    ...
    /**
     * Full {@see EduLevel} table as plain rows (forever cache; bust when levels are edited).
     *
     * @return list<array<string, mixed>>
     */
    public function getEduLevels(): array
    {
        return Cache::rememberForever(self::CACHE_KEY_EDU_LEVEL_ALL, function (): array {
            return EduLevel::query()
                ->orderBy('id')
                ->get()
                ->map(fn (EduLevel $m) => $m->getAttributes())
                ->values()
                ->all();
        });
    }
